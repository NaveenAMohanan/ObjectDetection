# -*- coding: utf-8 -*-
"""
Created on Sat Apr 21 11:10:55 2018
...
#reading and writing from folder ip and op
for i in range(10):
    
    if(i==0):
        print("Output written of image")
        continue
    print(i)
    ippath = str("ip/IP ("+str(i)+").tiff")
    oppath = str("op/OP ("+str(i)+").tiff")
    img = cv2.imread(ippath)
    out = detect(img,net.eval(),transform)
    cv2.imwrite(oppath,out)
#    cv2.imshow('Lena',out)
#    cv2.waitKey(0)
#    cv2.destroyAllWindows()
#uncomment above three lines to show each op on screen
